...
beforeEach(() => {
  // Re-create the test DB from scratch for each test
  db = {} as TestDb;
...
describe("TaskService", () => {
  ...
  async function fresh() {
    ...
    const svc = new TaskService(testDb as unknown as VNoteDatabase);
    return { db: testDb, service: svc };
  }
  ...
  describe("updateTask", () => {
    ...
    describe("repeating tasks", () => {
      it("daily repeat: completing advances dueDate by 1 day and keeps task active", async () => {
        const task = service.createTask({ title: "daily standup", listId: "inbox", dueDate: "2026-05-22" });
      ...
      it("weekly repeat: completing advances dueDate by 7 days", async () => {
        const task = service.createTask({ title: "weekly review", listId: "inbox", dueDate: "2026-05-22" });
      ...
      it("monthly repeat: completing advances dueDate by 1 month", async () => {
        const task = service.createTask({ title: "monthly report", listId: "inbox", dueDate: "2026-05-22" });
