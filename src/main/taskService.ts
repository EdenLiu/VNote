...
export class TaskService {
  ...
  setListSuggestions(listId: string, enabled: boolean): TaskList {
    this.db.run('UPDATE lists SET include_in_suggestions = ? WHERE id = ?', [
      enabled ? 1 : 0,
      listId,
    ]);
  ...
  updateList(listId: string, patch: TaskListPatch): TaskList {
    ...
    this.db.run('UPDATE lists SET name = ?, color = ?, include_in_suggestions = ? WHERE id = ?', [
      ...
      patch.includeInSuggestions !== undefined
        ? patch.includeInSuggestions
          ? 1
          : 0
        : current.includeInSuggestions
          ? 1
          : 0,
      listId,
    ]);
  ...
  getTasks(): Task[] {
    return this.db
      .select(
        'SELECT t.*, md.day my_day_date FROM tasks t LEFT JOIN my_day md ON md.task_id = t.id ORDER BY t.created_at DESC',
      )
  ...
  updateTask(taskId: string, patch: TaskPatch): Task {
    ...
    const dueDate =
      patch.dueDate === null
        ? undefined
        : addRepeatPeriod(patch.dueDate ?? current.dueDate, completingRepeat ? repeat : 'none');
    const reminderAt =
      patch.reminderAt === null
        ? undefined
        : addRepeatPeriod(
            patch.reminderAt ?? current.reminderAt,
            completingRepeat ? repeat : 'none',
          );
    ...
    const next = {
      ...
      tags:
        patch.tags ??
        extractTags(`${patch.title ?? current.title} ${patch.notes ?? current.notes}`),
    };
  ...
  addStep(taskId: string, draft: StepDraft): Step {
    const sortOrder = Number(
      this.db.select(
        'SELECT COALESCE(MAX(sort_order), -1) + 1 next_order FROM steps WHERE task_id = ?',
        [taskId],
      )[0]?.next_order ?? 0,
    );
    const step: Step = {
      id: newId(),
      taskId,
      title: draft.title.trim() || 'Untitled step',
      completed: false,
      sortOrder,
    };
    this.db.run('INSERT INTO steps VALUES (?, ?, ?, 0, ?)', [
      step.id,
      step.taskId,
      step.title,
      step.sortOrder,
  ...
  createAttachment(
    taskId: string,
    file: { name: string; size: number; storedPath: string; mimeType?: string },
  ): Attachment {
    ...
    return this.getTasks()
      .filter(
        (task) =>
          ...
          lists.get(task.listId)?.includeInSuggestions !== false,
      )
      .map((task) => ({
        task,
        score:
  ...
  private mapTask(row: Row): Task {
    ...
    return {
      ...
      tags: this.db
        .select('SELECT tag FROM task_tags WHERE task_id = ? ORDER BY tag ASC', [taskId])
        .map((tag) => String(tag.tag)),
      steps: this.db
        .select('SELECT * FROM steps WHERE task_id = ? ORDER BY sort_order ASC', [taskId])
        .map(mapStep),
      attachments: this.db
        .select('SELECT * FROM attachments WHERE task_id = ? ORDER BY created_at DESC', [taskId])
        .map(mapAttachment),
